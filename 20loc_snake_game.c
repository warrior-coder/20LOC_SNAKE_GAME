...
int main(int c, char* v[]) {
    for (CONSOLE_FONT_INFOEX cfi = { sizeof(cfi), 0, { 24, 24 }, 0, 0, L"Consolas" }; SetCurrentConsoleFontEx(GetStdHandle(STD_OUTPUT_HANDLE), 0, &cfi);) {
        ...
        for (struct { short y, x; } body[0x100] = { {10, 10}, {9, 10}, {8, 10}, {7, 10} }, apple = { (t1 >> 4U) % H, t1 % W }; game;) {
            if ((t2 = GetTickCount()), _kbhit()) if (_getch() == 0xE0) _ch = _getch();
            if ((int)(t2 - t1) > delay) {
                printf("\x1B[107m\x1B[%d;%dH \x1B[41m\x1B[%d;%dH ", body[size - 1].y + 1, body[size - 1].x + 1, apple.y + 1, apple.x + 1);
                ...
                printf("\x1B[102m\x1B[%d;%dH \x1B[42m\x1B[%d;%dH \x1B[20;20H", body[1].y + 1, body[1].x + 1, body->y + 1, body->x + 1);
            }
        }
    }
}